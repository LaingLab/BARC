...
class PDFViewer:
    ...
    def highlight_region(self, event):
        ...
        img_array[..., 3][mask] = 191  # Set alpha to 191 (~75% opacity)
    ...
    def count_cells(self):
        ...
        for prop in props:
            ...
            if 0 <= ax < atlas_img.width and 0 <= ay < atlas_img.height:
                mask_value = mask_img.getpixel((ax, ay))
                if mask_value > 0:
                    filtered_props.append(prop)
        
        # Count filtered
        num_cells = len(filtered_props)
        ...
        messagebox.showinfo("Cell Count", f"Estimated number of cells in selected regions: {num_cells}")
