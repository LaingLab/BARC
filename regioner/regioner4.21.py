...
class PDFViewer:
    def __init__(self, master):
        ...
        self.move_button.pack(side=tk.LEFT, padx=8, pady=8)

        ttk.Button(self.bottom_frame, text="Import TIFF", command=self.import_tiff).pack(side=tk.LEFT, padx=8, pady=8)

        # Rotation entry
        self.rotation_label = ttk.Label(self.bottom_frame, text="Rotate (degrees):")
        self.rotation_label.pack(side=tk.LEFT, padx=8, pady=8)
        self.rotation_entry = ttk.Entry(self.bottom_frame, width=8)
        self.rotation_entry.pack(side=tk.LEFT, padx=4, pady=8)
        ttk.Button(self.bottom_frame, text="Rotate", command=self.rotate_custom).pack(side=tk.LEFT, padx=8, pady=8)

        # Resize entry
        self.scale_label = ttk.Label(self.bottom_frame, text="Scale:")
        self.scale_label.pack(side=tk.LEFT, padx=8, pady=8)
        self.scale_entry = ttk.Entry(self.bottom_frame, width=8)
        self.scale_entry.pack(side=tk.LEFT, padx=4, pady=8)
        ttk.Button(self.bottom_frame, text="Resize Both Axes", command=self.resize_custom).pack(side=tk.LEFT, padx=8, pady=8)
        ...
        self.count_button.pack(side=tk.LEFT, padx=8, pady=8)
...
- Adjust the Sensitivity slider or enter a value (0-100) in the adjacent box to control the detection threshold; higher values detect more cells.
...
"""
    ...
    def show_page(self):
        img = self.load_page_image() or Image.new('RGBA', (1, 1), (0, 0, 0, 0))  # Fallback empty image
    ...
    def count_cells(self):
        ...
        img = np.array(self.original_background)
        ...
        for i, prop in enumerate(filtered_props, start=1):
            ...
            draw.text((int(x), int(y)), str(i), fill=(255, 0, 0, 77), font=font)
        ...
        self.last_df = pd.DataFrame({'Zone': zone_list, 'Cell_Count': count_list})
        save_path = fd.asksaveasfilename(title="Save Excel File", defaultextension=".xlsx", filetypes=[("Excel files", "*.xlsx")])
        if save_path:
            self.last_df.to_excel(save_path, index=False)
            messagebox.showinfo("Cell Counts Saved", f"Cell counts per zone saved to: {save_path}")
        else:
            # Create a new window to display the table
            table_win = tk.Toplevel(self.master)
            table_win.title("Cell Counts")

            tree = ttk.Treeview(table_win, columns=("Structure", "Count"), show="headings")
            tree.heading("Structure", text="Structure")
            tree.heading("Count", text="Count")
            tree.pack(expand=True, fill=tk.BOTH)

            for zone, count in zip(zone_list, count_list):
                tree.insert("", "end", values=(zone, count))
