...
class PDFViewer:
    ...
    def count_cells(self):
        ...
        gray = np.array(self.background_image.convert('L')).astype(float)

        # Apply Gaussian filter for smoothing
        smoothed = gaussian_filter(gray, sigma=1)

        # Compute threshold (adjust multiplier as needed for your images)
        mean = np.mean(smoothed)
        std = np.std(smoothed)
        thresh = mean + 0.5 * std  # Assuming bright cells on darker background; adjust if inverted
        ...
        messagebox.showinfo("Cell Count", f"Estimated number of cells: {num_features}")
