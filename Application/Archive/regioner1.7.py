...
class PDFViewer:
    ...
    def crop_end(self, event):
        ...
        crop_left = max(0, left - self.img_x)
        crop_top = max(0, top - self.img_y)
        crop_right = min(right - self.img_x, self.load_page_image().width)
        crop_bottom = min(bottom - self.img_y, self.load_page_image().height)
        # Crop the image
    ...
    def import_tiff(self):
        ...
        if tiff_path:
            ...
            array = np.array(tiff_img)
            if array.ndim == 2 or (array.ndim == 3 and array.shape[2] == 1):  # Grayscale
                array = np.squeeze(array)
                array_norm = (array - array.min()) / (array.max() - array.min() + 1e-8) * 255
                tiff_img = Image.fromarray(array_norm.astype(np.uint8)).convert('RGBA')
            elif array.max() <= 1.0:  # Float image
                array = (array * 255).astype(np.uint8)
                tiff_img = Image.fromarray(array).convert('RGBA')
            else:
                tiff_img = tiff_img.convert('RGBA')
            self.background_image = tiff_img
            self.show_page()
