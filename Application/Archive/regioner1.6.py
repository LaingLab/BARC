...
# FOS counter function (integrated from fos_counter.py)
def count_fos_positive(input_image, output_path=None):
    """
    ...
    Args:
        input_image (PIL.Image or np.array): Grayscale input image.
    ...
    coords = peak_local_max(distance, min_distance=5, exclude_border=True)
...
class PDFViewer:
    def __init__(self, master):
        self.master = master
        self.master.title('Combined Atlas Highlighter and Cell Counter')
        ...
        self.page_images = {}
        self.highlighted_masks = {}  # page: list of masks for highlighted regions
        ...
        # Background image (TIFF for cell counting)
        ...
        filemenu.add_command(label="Open Atlas (PDF/SVG)", command=self.open_file)
        ...
        ttk.Button(self.bottom_frame, text="Rotate Left", command=self.rotate_left).pack(side=tk.LEFT, padx=10, pady=10)
        ttk.Button(self.bottom_frame, text="Rotate Right", command=self.rotate_right).pack(side=tk.LEFT, padx=10, pady=10)
        ...
        ttk.Button(self.bottom_frame, text="Count Cells in Regions", command=self.count_cells_in_regions).pack(side=tk.LEFT, padx=10, pady=10)
    ...
    def crop_end(self, event):
        ...
        # Get crop coordinates in canvas pixels
        left = min(self.start_x, end_x)
        top = min(self.start_y, end_y)
        right = max(self.start_x, end_x)
        bottom = max(self.start_y, end_y)
        # Convert to PDF coordinates (divide by zoom)
        left_pdf = left / self.zoom
        top_pdf = top / self.zoom
        right_pdf = right / self.zoom
        bottom_pdf = bottom / self.zoom
        # Crop using clip rect at higher resolution
        page = self.doc[self.current_page]
        crop_rect = fitz.Rect(left_pdf, top_pdf, right_pdf, bottom_pdf)
        mat = fitz.Matrix(3.0, 3.0)  # Higher zoom for better quality
        pix = page.get_pixmap(matrix=mat, clip=crop_rect)
        mode = "RGBA" if pix.alpha else "RGB"
        cropped_img = Image.frombytes(mode, [pix.width, pix.height], pix.samples)
        if mode == "RGB":
    ...
    def import_tiff(self):
        ...
        if tiff_path:
            self.background_image = Image.open(tiff_path).convert("RGBA")
            self.show_page()
    ...
    def open_file(self):
        ...
        if self.path:
            ...
            self.current_page = 0
            self.page_images = {}
            self.highlighted_masks = {self.current_page: [] for _ in range(self.num_pages)}  # Initialize masks per page
            self.show_page()
    ...
    def highlight_region(self, event):
        ...
        # Save the mask for later counting
        if self.current_page not in self.highlighted_masks:
            self.highlighted_masks[self.current_page] = []
        self.highlighted_masks[self.current_page].append(mask)
    ...
    def count_cells_in_regions(self):
        if not self.background_image:
            messagebox.showerror("Error", "Please import a TIFF image first.")
            return
        if self.current_page not in self.highlighted_masks or not self.highlighted_masks[self.current_page]:
            messagebox.showerror("Error", "No highlighted regions found. Highlight regions first.")
            return

        # Assume atlas and TIFF are aligned and same size (user has adjusted)
        tiff_img = self.background_image.convert('L')  # Convert to grayscale for counting
        counts = []
        for i, mask in enumerate(self.highlighted_masks[self.current_page], 1):
            masked_tiff = np.zeros_like(tiff_img)
            masked_tiff[mask] = tiff_img[mask]
            count, _ = count_fos_positive(masked_tiff)
            counts.append((i, count))

        # Save to Excel
        df = pd.DataFrame(counts, columns=['Region ID', 'Cell Count'])
        save_path = fd.asksaveasfilename(defaultextension=".xlsx", filetypes=[("Excel files", "*.xlsx")])
        if save_path:
            df.to_excel(save_path, index=False)
            messagebox.showinfo("Success", f"Cell counts saved to {save_path}")
